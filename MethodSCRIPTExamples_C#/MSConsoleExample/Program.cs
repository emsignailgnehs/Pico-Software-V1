...
namespace EmStatConsoleExample
{
    class Program
    {
        ...
        static string FilePathSave = "C:\\Users\\aptitude\\Desktop\\Pico Software V1\\MethodSCRIPTExamples_C#\\MSConsoleExample"; //Change this path to where you want to drop the output files 
        ...
        readonly static Dictionary<string, string> MeasurementVariables = new Dictionary<string, string>  //Variable types and their corresponding labels
                                                                            { { "da", "E (V)" },
                                                                              ...
                                                                              { "cc", "Z' (Ohm)" },
                                                                              { "cd", "Z'' (Ohm)" } };
        readonly static Dictionary<int, string> SerialPicoTable = new Dictionary<int, string> { { 1, "EP1CA8GV" }, { 2, "EP1CA8HY" }, { 3, "EP1GB9RG" }, { 4, "EP2ICOPI" } };
        /// <summary>
        ...
        static int Main(string[] args)
        {
            ...
            if (canConvert)
            {
                int picoNumber = int.Parse(args[0]);
                SerialPortEsP = OpenSerialPort(picoNumber);

                if (SerialPortEsP != null)
                {
                    // Default Scan params if args == 0
                    string outputFilePath = "";
                    scanType = "Three Electrode Scan";
                    int numberOfElectrodes = 3;
                    string fluidFillbool = "False";
                    picoNumber = 1;
                    ...
                    if (args.Count() > 0)
                    {
                        // assign all args to variables
                        picoNumber = int.Parse(args[0]);
                        outputFilePath = args[1];
                        scanType = args[2];
                        numberOfScans = int.Parse(args[3]);
                        fluidFillbool = args[4];
                        ...
                        if (scanType.Equals("Continuous"))
                        {
                            continuous = true;
                        }

                        // Checks what type of scan is going to run and assign the number of electrodes to use accordingly
                        if (scanType.Equals("Three Electrode Scan"))
                        {
                            numberOfElectrodes = 3;
                        }
                        else if(scanType.Equals("Two Electrode Scan") || scanType.Equals("Two Electrode Scan (Alternating)"))
                        {
                            numberOfElectrodes = 2;
                        }
                        else if(scanType.Equals("One Electrode Scan") || continuous)
                        {
                            numberOfElectrodes = 1;
                        }
                    }

                    Console.WriteLine("\nConnected to EmStat Pico. " + picoNumber.ToString() + "\n");
                    // Fluid fill check
                    if (args.Count()>4 && fluidFillbool == "True")
                    {
                    ...
                    // Check if directory has continuous scanning files. If so, we can add to directory without overwriting
                    string[] dirs = Directory.GetFiles(outputFilePath, "WE" + args[0]+" *");
                    
                    // Code inside this for loop will only execute for numberOfElectrods == (3 || 1)
                    for (int i = 0; i < numberOfScans; i++)
                    {
                        ...
                        else if (numberOfElectrodes == 1)
                        {
                            SendScriptFile(W1FilePath);
                            ProcessReceivedPackages();
                            // Write csv as soon as one scan is done for continuous scanning
                            if (continuous)
                            {
                                writeCSVPath(picoNumber, outputFilePath, numberOfElectrodes,(dirs.Length + i).ToString());
                            }
                        }
                    }
                    ...
                    {
                        if (scanType.Equals("Two Electrode Scan (Alternating)"))
                        {
                            for (int i = 0; i < numberOfScans; i++)
                            {
                                SendScriptFile(W0FilePath);
                                ProcessReceivedPackages();
                                SendScriptFile(W2FilePath);
                                ProcessReceivedPackages();
                            }
                    ...
                    if(!continuous)
                    {
                        dataPoints = (NDataPointsReceived / numberOfElectrodes) / numberOfScans; //To get number of datapoints for one electrode scan
                        if (args.Count() != 0)
                            writeCSVPath(picoNumber, outputFilePath, numberOfElectrodes,"");
                        else
        ...
        static private void writeCSVPath(int picoNumber,string filePath, int numberOfElectrodes, string scanNumber)
        {
            ...
            if (numberOfElectrodes == 3)
            {
                string WE0csv = Path.Combine(filePath, "WE0" + picoNumber.ToString()+".csv");
                string WE1csv = Path.Combine(filePath, "WE1" + picoNumber.ToString() + ".csv");
                string WE2csv = Path.Combine(filePath, "WE2" + picoNumber.ToString() + ".csv");
                string WE0Vcsv = Path.Combine(filePath, "WE" + picoNumber.ToString() + "V.csv");
                WriteCSVCurrent(CurrentReadings.ToArray(), WE0csv, WE1csv, WE2csv);
                WriteCSVVoltage(VoltageReadings.ToArray(), WE0Vcsv);
            }
            else if (numberOfElectrodes == 2)
            {
                string WE0csv = Path.Combine(filePath, "WE0" + picoNumber.ToString() + ".csv");
                string WE2csv = Path.Combine(filePath, "WE2" + picoNumber.ToString() + ".csv");
                string WE0Vcsv = Path.Combine(filePath, "WE" + picoNumber.ToString() + "V.csv");
                if (scanType.Equals("Two Electrode Scan (Alternating)"))
                {
                    WriteCSVCurrentTwoElectrode(CurrentReadings.ToArray(), WE0csv, WE2csv);
                    WriteCSVVoltage(VoltageReadings.ToArray(), WE0Vcsv);
                }
                else
                {
                    double[] we0Vector = CurrentReadings.GetRange(0, (NDataPointsReceived / 2)).ToArray();
                    double[] we2Vector = CurrentReadings.GetRange(NDataPointsReceived / 2, (NDataPointsReceived / 2)).ToArray();
                    WriteCSVCurrentOneElectrode(we0Vector, WE0csv);
                    WriteCSVCurrentOneElectrode(we2Vector, WE2csv);
                    WriteCSVVoltage(VoltageReadings.ToArray(), WE0Vcsv);
                }
            ...
            else if (numberOfElectrodes == 1)
            {
                ...
                if (continuous)
                {
                    string WEcsv = Path.Combine(filePath, "WE" + picoNumber.ToString() + " " + scanNumber + ".csv");
                    WriteCSVContinuous(timestampArray.ToArray(), CurrentReadings.ToArray(), VoltageReadings.ToArray(), WEcsv);
                }
                else
                {
                    string WEcsv = Path.Combine(filePath, "WE"+ picoNumber.ToString() + ".csv");
                    string WEVcsv = Path.Combine(filePath, "WE" + picoNumber.ToString() + "V.csv");
                    WriteCSVCurrentOneElectrode(CurrentReadings.ToArray(), WEcsv);
                    WriteCSVVoltage(VoltageReadings.ToArray(), WEVcsv);
                }
        ...
        private static void WriteCSVCurrentOneElectrode(double[] vector, String weFilePath)
        {
            // Note dataPoints is how many points for one scan
            double[,] we = new double[dataPoints, numberOfScans];
            string line = "";
            for (int i = 0; i < numberOfScans; i++)
            {
                for (int j = 0; j < dataPoints; j++)
                {
                    we[j, i] = vector[j + (dataPoints * i)];
                }
            }

            TextWriter weCSV = new StreamWriter(weFilePath);
            for (int row = 0; row < dataPoints; row++)
            {
                for(int column = 0; column < numberOfScans; column++)
                {
                    line = line + we[row, column].ToString() + ",";
                }
                weCSV.WriteLine(line);
                line = "";
            }
            weCSV.Close();
        }

        private static void WriteCSVCurrentTwoElectrode(double[] vector, String we0FilePath,String we2FilePath)
        {
            ...
            for (int i = 0; i < numberOfScans; i++)
            {
                for (int j = 0; j < dataPoints; j++)
                {
                    we0[j, i] = vector[j + (dataPoints * i * 2)];
                    we2[j, i] = vector[(dataPoints + j) + (dataPoints * i * 2)];
                }
            }
            ...
            for (int row = 0; row < dataPoints; row++)
            {
                for (int column = 0; column < numberOfScans; column++)
                {
                    line = line + we0[row, column].ToString() + ",";
                }
                ...
                for (int column = 0; column < numberOfScans; column++)
                {
                    line = line + we2[row, column].ToString() + ",";
                }
        ...
        private static void WriteCSVCurrent(double[] vector,String we0FilePath,String we1FilePath,String we2FilePath)
        {
            ...
            for (int row = 0; row < dataPoints; row++)
            {
                ...
                for (int column = 0; column < numberOfScans; column++)
                {
                    line = line + we1[row, column].ToString() + ",";
                }
                we1CSV.WriteLine(line);
                line = "";
        ...
        {
            double[,] voltage = new double[dataPoints, numberOfScans];
            string line = "";
            for (int i = 0; i < numberOfScans; i++)
            {
                for (int j = 0; j < dataPoints; j++)
                {
                    voltage[j, i] = vector[j + (dataPoints * i)];
                }
            }

            TextWriter voltageCSV = new StreamWriter(we0VFilePath);
            for (int row = 0; row < dataPoints; row++)
            {
                for (int column = 0; column < numberOfScans; column++)
                {
                    line = line + voltage[row, column].ToString() + ",";
                }
                voltageCSV.WriteLine(line);
                line = "";
            }
            voltageCSV.Close();
        }
        ...
        private static SerialPort OpenSerialPort(int picoNumber)
        {
                ...
                if (SerialPortEsP == null && !serialPort.IsOpen)
                {
                    try
                    {
                        ...
                        if (serialPort.IsOpen)
                        {
                            ...
                            serialPort.Write("i\n");
                            string serialNumber = SerialPicoTable[picoNumber];
