...
def fitpeak(xydataIn, nstd, peakwidth, minpot, maxpot):
    ...
    centerLocs, peakProps = signal.find_peaks(truncy, height=0,width=0,rel_height=0.5)
   
    if len(peakProps["peak_heights"]) > 2:
        prom = 0 
        while len(peakProps["peak_heights"]) > 2:
            prom = prom + .01
            centerLocs, peakProps = signal.find_peaks(truncy,height=100*(10**(-9)),prominence=prom,width=100*(10**(-3)),rel_height=0.5)
    ...
    peakHeights = numpy.delete(peakHeights,[i for i in range(len(centerLocs)) if centerLocs[i]==0])
    peakwidths = numpy.delete(peakwidths,[i for i in range(len(centerLocs)) if centerLocs[i]==0])
    centerLocs = numpy.delete(centerLocs,[i for i in range(len(centerLocs)) if centerLocs[i]==0])
    
    peakHeight = 0
    if(len(peakHeights) > 0):
        peakHeight = max(peakHeights)
    centerLoc = centerLocs[[i for i in range(len(peakHeights)) if peakHeights[i]==peakHeight]]
    peakwidth = peakwidths[[i for i in range(len(peakHeights)) if peakHeights[i]==peakHeight]]

    if len(centerLoc) > 1:
        centerLoc = min(centerLoc)
        peakwidth = sum(peakwidths[[i for i in range(len(peakHeights)) if peakHeights[i]==peakHeight]])
    ...
    if len(centerLoc) == 0:
        ...
        for i in range(0, len(truncx)):
            ...
            baseMask.append((truncx[i] > (center + peakwidth/2.5)) or (truncx[i] < (center - peakwidth/2.5)))
            
            # Gets the portion aroud the center (1/2 peak width) that's the gaussian
            peakMask.append((truncx[i] > (center - peakwidth/1.5)) and (truncx[i] < (center + peakwidth/1.5))) 
        ...
        v0 = [vp[0],vp[1],numpy.log(peakHeight-linFit(vp,center))[0],center[0],peakwidth/6]
        ...
        peakMask = []
        for i in range(0, len(truncx)):
            peakMask.append((truncx[i] > (v[3] - nstd*v[4])) and (truncx[i] < (v[3] + nstd*v[4]))) 
        
        truncxMasked = x[[i for i in range(len(peakMask)) if peakMask[i]==True]]
...
def gaussAndLinFit(v,x):
    ...
    for i in range(len(x)):
        y.append(v[0]*x[i] + v[1] + numpy.exp(v[2])*numpy.exp(-(((x[i]-v[3])**2)/(2*v[4]**2))))
    return y
...
def getFillFitError(v,x,y):
    ...
    for i in range(0, len(x)):
            peakMask.append((x[i] > (v[3] - 2.5*v[4])) and (x[i] < (v[3] + 2.5*v[4])))
            baseMask.append((x[i] > (v[3] - nstd*v[4])) and (x[i] < (v[3] + nstd*v[4])) and (not peakMask[i])) 
    ...
    productArray = numpy.multiply(subGaussAndLin/numpy.sqrt(sum(peakMask)),multGuassNorm)
